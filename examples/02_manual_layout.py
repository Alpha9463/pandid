...
The two trains draw the same geometry, the top pinned by the corner and
the bottom by the nozzle. ``via([...])`` is the third override:
explicit orthogonal waypoints for one stream.
"""
...
def main():
    ...
    # Each nozzle lands wherever its own symbol puts it, so these
    # numbers only line up against today's artwork.
    ...
    # and each unit hung off it by nozzle; stepping along the row is
    # still the corner's job, so the second pin() names only y.
    run_y = 330
