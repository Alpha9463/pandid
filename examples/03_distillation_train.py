...
Two towers, each with a condenser onto a reflux drum whose single draw parts
into reflux and distillate, and a kettle reboiler off the sump returning
boilup, with the net bottoms taken over the weir. Both loops close on the
column's ``reflux_in`` and ``boilup_in`` nozzles. 06_column_reflux draws one
...
def main():
    ...
    # own nozzle sits, so no half-height is written down here.
    col_y = 420
    ...
    # A boundary flag is pinned at the tip of its arrow, and this one's line
    # lands on in_1, which is above the outlet the run is pinned on.
    ...
    # Both towers carry the same rows above and below them, so each row is
    # named once here and every device on it is pinned to it by its own nozzle.
    ...
    # Each overhead and bottoms system hangs off its own tower's centreline,
    # asked of the symbol rather than measured off the drawing. The two return
    # elevations are never written down: reflux_in and boilup_in sit at fixed
    # fractions of whatever height the column is drawn at, so connecting by
    # name keeps that fraction out of the script.
    c1_axis = col1.pin_.x + port_offset(col1, "distillate")[0]
    c2_axis = col2.pin_.x + port_offset(col2, "distillate")[0]

    # Condenser over its own tower on the same axis, flipped top to bottom so
    # the vapour rises into its underside dead straight. The vapour takes the
    # shell's N-S pair and the cooling water the tubes' W-E one.
    ...
    # Drum east of its tower, and nozzle() moves the inlet to the top face:
    # left alone the engine reaches for the head, which would hook the drop off
    # the condenser back on itself. drum_y also buys the draw its room -- a Tee
    # draws no symbol, so the pipe either side of it is the only thing saying
    # one is there, and at 52 units the draw parts clear of the drum.
    ...
    # page and the flip puts the branch out east.
    ...
    # The tee is inline, so the drum's draw and the reflux leg it runs into
    # carry one number and only the branch takes one of its own.
