...
def _biodiesel_plant() -> Flowsheet:
    """Example 22 -- batch transesterification, and the only batch sheet here.

    The sheet that made a draw.io cycle reachable: two runs that each cross
    ...
    meoh_feed = fs.add(units.Feed("Methanol", reference="PFD-002"))
    oil_feed = fs.add(units.Feed("Canola Oil", reference="PFD-001"))
    acid_feed = fs.add(units.Feed("Phosphoric Acid", reference="PFD-004"))
    ww_feed = fs.add(units.Feed("Wash Water", reference="PFD-005"))
    hw_supply = fs.add(units.Feed("HWSH", header=True))
    steam_reb = fs.add(units.Feed("LPSSH", header=True))
    steam_ej = fs.add(units.Feed("LPSSH", header=True))
    steam_condensate = fs.add(units.Product("LPSRH", header=True))
    cws = fs.add(units.Feed("CWSH", header=True))
    cwr = fs.add(units.Product("CWRH", header=True))
    reactor_vent = fs.add(units.Product("Reactor Vent to Vapour Recovery", reference="P&ID-701"))
    wastewater = fs.add(units.Product("Wash Water to Wastewater Treatment", reference="P&ID-702"))
    vacuum_vent = fs.add(units.Product("Vacuum System Discharge", reference="P&ID-703"))
    biodiesel_prod = fs.add(units.Product("Biodiesel (FAME) to Loading", reference="P&ID-801"))
    glycerol_prod = fs.add(units.Product("Crude Glycerol to Loading", reference="P&ID-802"))

    # --- Equipment ------------------------------------------------------
    # Oil receipt and storage.
    tk401 = fs.add(units.Tank("TK-401", description="Canola Oil Storage Tank"))
    p401 = fs.add(units.Pump("P-401", description="Oil Charge Pump"))
    ...
    tk402 = fs.add(units.Tank("TK-402", description="Methanol Storage Tank"))
    p402 = fs.add(units.Pump("P-402", description="Methanol Charge Pump"))
    bv401 = fs.add(
        units.Vent("BV-401", variant="breather", description="Methanol Tank Conservation Vent")
    )
    mt401 = fs.add(
        units.Reactor(
            "MT-401", n_feeds=2, width=71.04, height=151, description="Methoxide Mixing Tank"
        )
    )
    fn401 = fs.add(units.Funnel("FN-401", description="KOH Catalyst Charging Funnel"))
    ...
    p403 = fs.add(units.Pump("P-403", description="Reactor Discharge Pump"))
    s401 = fs.add(
        units.Separator("S-401", characteristic="gravity", description="Glycerol Gravity Settler")
    )
    ...
    c401 = fs.add(
        units.Column("C-401", internals="packing", trays=2, description="Methanol Stripping Column")
    )
    e401 = fs.add(
        units.HeatExchanger(
            "E-401", variant="straight_tubes", description="Methanol Overhead Condenser"
        )
    )
    d401 = fs.add(units.Vessel("D-401", variant="horizontal", description="Methanol Reflux Drum"))
    e402 = fs.add(devices.KettleReboiler("E-402", description="Ester Reboiler"))

    # Washing, vacuum drying and product storage.
    m401 = fs.add(units.Mixer("M-401", n_inlets=2, description="Wash Water Mixer"))
    s402 = fs.add(
        units.Separator("S-402", characteristic="gravity", description="Wash Water Separator")
    )
    s403 = fs.add(units.Separator("S-403", description="Vacuum Flash Dryer"))
    ej401 = fs.add(units.Ejector("EJ-401", description="Dryer Vacuum Ejector"))
    p404 = fs.add(units.Pump("P-404", description="Biodiesel Loading Pump"))
    tk403 = fs.add(units.Tank("TK-403", description="Biodiesel Product Storage Tank"))

    # Glycerol neutralisation and by-product storage.
    n401 = fs.add(
        units.Reactor(
            "N-401", n_feeds=2, width=71.04, height=151, description="Glycerol Neutralisation Tank"
        )
    )
    p405 = fs.add(units.Pump("P-405", description="Glycerol Transfer Pump"))
    tk404 = fs.add(units.Tank("TK-404", description="Crude Glycerol Storage Tank"))

    # --- In-line devices -------------------------------------------------
    hv402 = fs.add(units.Valve("HV-402", description="Jacket Hot Water Block Valve"))
    fe402 = fs.add(
        units.Fitting(
            flow402.element("FE"), variant="venturi", description="Jacket Hot Water Flow Element"
        )
    )
    cv402 = fs.add(
        units.Valve(
    ...
    cv403 = fs.add(
        units.Valve(
            level403.tag("CV"), variant="control", description="Glycerol Draw Control Valve"
        )
    )
    hv403 = fs.add(units.Valve("HV-403", description="Condenser Cooling Water Block Valve"))
    fe405 = fs.add(
        units.Fitting(flow405.element("FE"), variant="venturi", description="Reflux Flow Element")
    )
    cv405 = fs.add(
        units.Valve(flow405.tag("CV"), variant="control", description="Reflux Control Valve")
    )
    t_meoh_in = fs.add(units.Tee(branch="inlet"))  # recovered methanol rejoins the fresh feed
    t_reflux = fs.add(units.Tee())  # the reflux splits off the drum draw
    cv406 = fs.add(
        units.Valve(
            level406.tag("CV"),
            variant="control",
    ...
    hv404 = fs.add(units.Valve("HV-404", description="Reboiler Steam Block Valve"))
    cv407 = fs.add(
        units.Valve(
            level407.tag("CV"), variant="control", description="Stripped Ester Draw Control Valve"
        )
    )
    cv408 = fs.add(
        units.Valve(
            level408.tag("CV"), variant="control", description="Wash Water Draw Control Valve"
        )
    )
    ...
    fs.connect(
        oil_feed.outlet, tk401.inlet, service="CO", sequence=401, size=150, schedule=40, spec="CS"
    )
    fs.connect(
        tk401.outlet, p401.suction, service="CO", sequence=402, size=100, schedule=40, spec="CS"
    )
    fs.connect(
        p401.discharge, r401.feeds[0], service="CO", sequence=403, size=80, schedule=40, spec="CS"
    )
    ...
    fs.connect(
        fn401.outlet, mt401.feeds[1], service="KOH", sequence=407, size=25, schedule=40, spec="SS"
    )
    fs.connect(
        mt401.outlet, r401.feeds[1], service="MX", sequence=408, size=50, schedule=40, spec="SS"
    )

    # The batch reaction and the phase split.
    fs.connect(
        r401.vent, reactor_vent.inlet, service="RV", sequence=409, size=50, schedule=40, spec="SS"
    )
    fs.connect(
        r401.outlet, p403.suction, service="RM", sequence=410, size=80, schedule=40, spec="SS"
    )
    fs.connect(
        p403.discharge, s401.feed, service="RM", sequence=411, size=80, schedule=40, spec="SS"
    )
    ...
    fs.connect(
        acid_feed.outlet, n401.feeds[1], service="PA", sequence=414, size=25, schedule=40, spec="SS"
    )
    ...
    fs.connect(
        d401.outlet, t_reflux.inlet, service="RCM", sequence=419, size=50, schedule=40, spec="SS"
    )
    fs.connect(
    ...
    fs.connect(
        ww_feed.outlet, m401.in_2, service="WW", sequence=427, size=40, schedule=40, spec="CS"
    )
    fs.connect(m401.outlet, s402.feed, service="WM", sequence=428, size=50, schedule=40, spec="SS")
    fs.connect(
        s402.port("overflow"),
        s403.feed,
        service="WE",
    ...
    fs.connect(
        s403.vapor, ej401.suction, service="WV", sequence=431, size=50, schedule=40, spec="CS"
    )
    fs.connect(
        steam_ej.outlet, ej401.motive, service="LPS", sequence=432, size=25, schedule=40, spec="CS"
    )
    fs.connect(
        ej401.discharge,
        vacuum_vent.inlet,
    ...
    fs.connect(
        s403.liquid, p404.suction, service="BD", sequence=434, size=50, schedule=40, spec="SS"
    )
    fs.connect(
        p404.discharge, tk403.inlet, service="BD", sequence=435, size=50, schedule=40, spec="SS"
    )
    fs.connect(
        tk403.outlet,
        biodiesel_prod.inlet,
        service="BD",
        sequence=436,
        size=80,
        schedule=40,
    ...
    fs.connect(
        n401.outlet, p405.suction, service="NG", sequence=437, size=50, schedule=40, spec="SS"
    )
    fs.connect(
        p405.discharge, tk404.inlet, service="NG", sequence=438, size=50, schedule=40, spec="SS"
    )
    fs.connect(
        tk404.outlet,
        glycerol_prod.inlet,
    ...
    fs.connect(
        hw_supply.outlet, hv402.inlet, service="HWS", sequence=440, size=50, schedule=40, spec="CS"
    )
    ...
    tic404 = fs.add_instrument("TIC", temp404, near=tt404, at="N", offset=70, variant="shared")
    ...
    fic405 = fs.add_instrument("FIC", flow405, near=ft405, at="N", offset=150, variant="shared")
    ...
    lic408 = fs.add_instrument("LIC", level408, near=cv408, at="E", offset=70, variant="shared")
    ...
    fs.add_annotation(
        notes(
            [
                ...
                "One wash stage is drawn; a production design washes the ester two to three times.",
            ],
            title="GENERAL NOTES",
            numbered=False,
            align="bottom-left",
...
SCENARIOS = {
    ...
    # The only batch sheet, and the one that reaches the draw.io hop cycle.
    # Its title block states its own date, so nothing here is pinned.
    "22_biodiesel_plant": (_biodiesel_plant, {"border": "zone", "diagram": "p&id"}),
}
