...
def test_layering():
    ...
    assert u1.placement.col == 0
    assert u2.placement.col == 1
    assert u3.placement.col == 2
...
def test_pinned_layering():
    ...
    assert u1.placement.col == 2
    # u2 must be at least u1.col + 1
    assert u2.placement.col == 3
...
def test_ordering():
    ...
    assert u1.placement.row == 0
    assert {u2.placement.row, u3.placement.row} == {0, 1}
...
def test_full_layout_via_render(tmp_path):
    ...
    # rendering should implicitly trigger layout because placements are None
    svg_path = tmp_path / "test.svg"
    fs.render(str(svg_path))
    
    assert u1.placement.x is not None
    assert u2.placement.col == 1
