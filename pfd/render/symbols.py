...
- **Vendored (Equinor engineering-symbols, MIT)** — valves and their variants,
  pumps, compressors, blowers, vessels, tanks, reducers, nozzles, and
  instrument bubbles. Generated by ``scripts/vendor_symbols.py`` into
  ``_vendored_symbols.py`` and registered last (overriding the hand-drawn
  defaults of the same kind). See the repo ``NOTICE`` for attribution.
- **Hand-drawn primitives** — Feed/Product boundary markers and the symbols
  Equinor doesn't fit cleanly (multi-port shell-and-tube exchanger, tray
  column, reactor, separator, heater, cooler, mixer, splitter).
